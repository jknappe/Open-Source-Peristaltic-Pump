...
byte low, high;
  ...
  low=wert&0xFF;
  high=(wert>>8)&0xFF;
  EEPROM.write(adr, low); // dauert 3,3ms// include the library code:
#include <LiquidCrystal.h> //https://www.arduino.cc/en/Reference/LiquidCrystal -> LCD control
#include <ClickEncoder.h> //https://github.com/0xPIT/encoder/blob/master/README.md -> Encoder processing (timer based)
#include <TimerOne.h> //required for ClickEncoder.h 
#include <EEPROM.h> //write and read EEPROM (to save and load settings)
...
char inChar;
int usb_state = 0;               // state of USB command: 0=idle, 1=pump, 2=dose, 3=calibrate
unsigned long dose_start = 0;    // stores time when USB dosing starts (millisecs)
unsigned long dose_end = 0;      // stores time when USB dosing stops (millisecs)
unsigned long pump_start = 0;    // stores time when USB pumping starts (millisecs)
unsigned long pump_end = 0;      // stores time when USB pumping stops (millisecs)
...
if (in_action){
  ...
  while (Serial.available()) {
    ...
    if (inChar == 'p'){
      ...
      usb_state = 1;                        // set USB state to "pump"
      pump_start = millis();                // time when USB pumping starts (millisecs)
        Serial.print("supplied_pump,");     // and print to terminal in format:
        Serial.print("NA");                 // pump,NA,[rate],[cal],[pump_runtime_millisecs]
        Serial.print(",");
        Serial.print(rate_uL_min);
        Serial.print(",");
        Serial.print(cal);
      ...
      usb_state = 2;                        // set USB state to "dose"
      dose_start = millis();                // time when USB dosing starts (millisecs)
        Serial.print("supplied_dose,");     // and print to terminal in format:
        Serial.print(vol_uL);               // supplied_dose,[volume],[rate],[cal],[pump_runtime_millisecs]
        Serial.print(",");
        Serial.print(rate_uL_min);
        Serial.print(",");
        Serial.print(cal);
      ...
      usb_start=true;
      usb_state = 3;                        // set USB state to "calibrate" (currently unused)
    } else if (inChar == 'w'){
      cal=Serial.parseInt();
      menu[7].value =cal;
      for (int i=0; i <= menu_items_limit; i++){
      ...
      if (usb_state == 1) {                 // if pumping via USB command and "x" is received
        pump_end = millis();                // write end time (millisecs) to terminal
        Serial.println(pump_end - pump_start);
        }
      usb_state = 0;                        // set USB state to "idle"      
    }
  }
  ...
  if (usb_start) {
    if(inChar == 'p'){
      ...
      if (dose(steps, delay_us, step_counter)){
        ...
        usb_state = 0;                      // set USB state to "idle"  
        dose_end = millis();
          Serial.println(dose_end - dose_start);
      }
    } else if (inChar == 'c'){
      if (dose(CALIBR_STEPS, CALIBR_DELAY_US, step_counter)){
        usb_start = false;
